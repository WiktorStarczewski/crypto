...
impl InstanceShapes {
    /// Cross-check instances against the shape metadata and return the
    /// validated max log trace height. AIR-level invariants (counts, periodic
    /// columns, ascending order, non-empty, …) are enforced here; LDE-bound
    /// checks live in [`LiftedDomain::canonical`] and surface separately as
    /// [`crate::DomainError`] when domains are constructed.
    ///
        ...
        A: LiftedAir<F, EF>,
    {
        ...
        // Bound `log_h` so the `1usize << log_h` below cannot overflow on this
        // target. Field-relative bounds (against `F::TWO_ADICITY`) are enforced
        // separately by [`LiftedDomain::canonical`] when domains are built.
