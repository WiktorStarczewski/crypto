...
impl<F: TwoAdicField> PeriodicLde<F> {
    ...
    ///
    /// # Arguments
    /// - `domain`: The lifted domain providing trace and LDE coset information
    ...
    pub fn build(
        ...
        repeated_matrix: Option<RowMajorMatrix<F>>,
    ) -> Self {
        ...
        let log_blowup = log_constraint_degree as usize;
        ...
        let period_shift: F = domain.lde_shift().exp_power_of_2(log_ratio as usize);
...
mod tests {
    ...
    fn assert_periodic_lde_matches_full(
        ...
        // Test treats the LDE blowup as the constraint degree (max case).
        let periodic_lde = PeriodicLde::build(&domain, log_blowup, Some(repeated_matrix));
        ...
        let expected_shift = domain.lde_shift();
