...
/// Groups multiple AIRs with the same constraint degree before upsampling.
/// Tests that the prover correctly cyclic-extends and accumulates traces of
/// different heights within the same degree bucket.
#[test]
fn upsample_groups_same_degree_airs() {
    let low1 = PowerAir { power: 2 };
    let low2 = PowerAir { power: 2 };
    let high = PowerAir { power: 9 };

    // Trace heights must be non-decreasing
    let t_low1 = generate_pow_trace(2, Felt::from_u64(7), 16);
    let t_low2 = generate_pow_trace(2, Felt::from_u64(13), 32);
    let t_high = generate_pow_trace(9, Felt::from_u64(11), 64);

    let w_low1 = AirWitness::new(&t_low1, &[], &[]);
    let w_low2 = AirWitness::new(&t_low2, &[], &[]);
    let w_high = AirWitness::new(&t_high, &[], &[]);

    let builder = TrivialAuxBuilder;
    prove_and_verify_instances(&[
        (&low1, w_low1, &builder),
        (&low2, w_low2, &builder),
        (&high, w_high, &builder),
    ]);
...
/// Groups AIRs whose executions are interleaved with a different-degree AIR.
/// Two D=2 AIRs share a bucket but are separated in instance order by a D=4 AIR;
/// each must be scaled by its own `beta^(n-1-j)` coefficient regardless of when
/// it is added to the bucket.
#[test]
fn upsample_groups_with_execution_gap() {
    let low1 = PowerAir { power: 2 };
    let mid = PowerAir { power: 5 };
    let low2 = PowerAir { power: 2 };
    let high = PowerAir { power: 9 };

    // Trace heights must be non-decreasing
    let t_low1 = generate_pow_trace(2, Felt::from_u64(7), 16);
    let t_mid = generate_pow_trace(5, Felt::from_u64(17), 16);
    let t_low2 = generate_pow_trace(2, Felt::from_u64(13), 32);
    let t_high = generate_pow_trace(9, Felt::from_u64(11), 64);

    let w_low1 = AirWitness::new(&t_low1, &[], &[]);
    let w_mid = AirWitness::new(&t_mid, &[], &[]);
    let w_low2 = AirWitness::new(&t_low2, &[], &[]);
    let w_high = AirWitness::new(&t_high, &[], &[]);
